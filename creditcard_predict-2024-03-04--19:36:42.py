...
# 定义创建 LSTM 模型的函数
def create_lstm_model(units, dropout, learning_rate, input_shape, epochs, batch_size, optimizer):
    model = Sequential()
...
random_search = RandomizedSearchCV(
    ...
    n_iter=20,
    scoring=make_scorer(custom_scorer, greater_is_better=True),  # using custom scorer
    cv=tscv,
    n_jobs=-1
)

# 执行 RandomizedSearchCV 搜索
