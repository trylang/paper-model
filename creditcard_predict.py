...
df_X = data.iloc[:, data.columns != 'Class']
df_y = data.iloc[:, data.columns == 'Class']
...
tree = DecisionTreeClassifier(random_state=SEED) #决策树模型

svm = SVC(probability=True,random_state=SEED,tol=1e-6)  # SVM模型

forest=RandomForestClassifier(n_estimators=100,random_state=SEED) #　随机森林
...
def plot_learning_curve(model, X, y, scorer):
    train_sizes, train_scores, test_scores = learning_curve(model, X, y, train_sizes=np.linspace(0.1, 1.0, 10), cv=5, scoring=scorer)
...
plot_validation_curve(best_model, X_test_undersample, y_test_undersample, 'n_estimators', [100, 200, 300, 400, 500], scorer)
